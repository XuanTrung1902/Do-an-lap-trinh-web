...
//    public List<Product> getProductsByBrands1(String[] brands) {
//        Jdbi jdbi = JDBIConnect.get();
//        String sql = """
//                    SELECT
//                        p.id, p.name, p.des, p.price, p.quantity, p.version, p.launch,
//                        p.status, b.name AS brand, min(i.url) AS imgUrl
//                    FROM products AS p
//                    JOIN imgs AS i ON i.productID = p.id
//                    JOIN brands AS b ON p.brandID = b.id
...
//                        .mapToBean(Product.class) // Ánh xạ trực tiếp vào lớp Product
//                        .list()
//        );
//    }
